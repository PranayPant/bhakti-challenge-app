...
export const ChallengeStoreContext = createContext<ReturnType<
  typeof createChallengeStore
> | null>(null);
...
export function ChallengeStoreProvider({
  children,
  initialStore,
...
export function useChallengeStore<U>(selector: (state: ChallengeStore) => U) {
  ...
  if (!store) {
    throw new Error(
      "useChallengeStore must be used within ChallengeStoreProvider"
    );
  }
