...
function processRawText() {
  try {
    if (fs.existsSync("data/hindi-challenges-raw.txt")) {
      const rawHindiText = fs.readFileSync(
        "data/hindi-challenges-raw.txt",
        "utf8"
      );
...
function main() {
  ...
  fs.writeFileSync(
    "data/english-challenges.json",
    JSON.stringify(jsonEnglishChallenges, null, 2),
    { flag: "w+" }
  );
  fs.writeFileSync(
    "data/hindi-challenges.json",
    JSON.stringify(jsonHindiChallenges, null, 2),
    { flag: "w+" }
  );
}
